...
changed_cost['Cost Basis'] = changed_cost['Quantity'] * \
    changed_cost['Unit Cost']
changed_cost = changed_cost[['Acquisition Date', 'Ticker',
                             'Quantity', 'Unit Cost', 'Cost Basis', 'Start of Year']]
...
def get_standard_deviation():
    ...
    for ticker in tickers:
        ...
        volatility = singled_tickers['Adj Close'].pct_change(
        ).std() * (252 ** 0.5)
...
merged_portfolio = pd.merge(
    changed_cost, adj_close_latest, left_index=True, right_index=True)
merged_portfolio['Ticker Return'] = merged_portfolio['Adj Close'] / \
    merged_portfolio['Unit Cost'] - 1
# reset index to get tickers back as separate column
...
merged_portfolio_sp['Equiv SP Shares'] = merged_portfolio_sp['Cost Basis'] / \
    merged_portfolio_sp['SP 500 Initial Close']
merged_portfolio_sp['Latest Date'] = merged_portfolio_sp['Latest Date'].astype(
    'datetime64[D]')
...
merged_portfolio_sp_latest_YTD.rename(
    columns={'Adj Close': 'Ticker Start Year'}, inplace=True)
...
merged_portfolio_sp_latest_YTD_sp.rename(
    columns={'Adj Close': 'SP Start Year'}, inplace=True)
...
portfolio = merged_portfolio_sp_latest_YTD_sp.sort_values(
...
portfolio['Cum Ticker ROI'] = portfolio['Cum Ticker Return'] / \
    portfolio['Cum Investment']
# print(portfolio)
...
adj_close_acq_date.sort_values(by=['Ticker', 'Acquisition Date', 'Date'], ascending=[
                               True, True, True], inplace=True)

# * Get timedelta to look at highest close which occurred after acquisition date
adj_close_acq_date['Date Delta'] = adj_close_acq_date['Date'].subtract(
    adj_close_acq_date['Acquisition Date'])
adj_close_acq_date['Date Delta'] = adj_close_acq_date[[
    'Date Delta']].apply(pd.to_numeric)  # strings to numeric
...
adj_close_pivot_merged = pd.merge(
    adj_close_pivot, adj_close, on=['Ticker', 'Adj Close'])
# print(adj_close_pivot_merged)

# * Merge adj close pivot table with adj_close table to grab date of adj close high
final_portfolio = pd.merge(portfolio, adj_close_pivot, on=[
                           'Ticker', 'Acquisition Date'])
final_portfolio.rename(columns={
                       'Adj Close': 'Highest Adj Close', 'Date': 'Highest Adj Close Date'}, inplace=True)
final_portfolio['% off High'] = final_portfolio['Ticker Adj Close'] / \
    final_portfolio['Highest Adj Close'] - 1
...
final_portfolio['Pct of portfolio'] = final_portfolio['Share Value'] / \
    final_portfolio['Cum Ticker Return'].tail(1).values[0]
...
layout = go.Layout(title='Adj Close % off High',
                   title_x=0.5,
                   barmode='group',
                   yaxis=dict(title='% Below Adj Close High',
                              tickformat='.2%'),
                   xaxis=dict(title='Ticker'),
                   legend=dict(x=.8, y=1),
                   template='simple_white',
...
layout = go.Layout(title='Total Cumulative Investments and Returns Over Time',
                   ...
                   xaxis=dict(title='Ticker'),
                   legend=dict(x=.4, y=1),
                   yaxis2=dict(title='Cum ROI Mult',
